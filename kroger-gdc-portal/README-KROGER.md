...
3. **Configure Store VLAN Networks & IPAM**: Scroll down to the **Store VLAN Networks & IPAM** section.
   - Enter your VLANs (e.g. Non-PCI, PCI) including custom VLAN Name, ID, Subnet, Gateway, VIP Range, and Pod CIDR.
   - Use the **+ Add VLAN** button to configure multiple VLAN subnets.
4. **Deploy Virtual GDC Environment**: Click **Deploy Virtual GDC Environment**. The stepper will provision the virtual cluster infrastructure, register the GKE Hub memberships, and automatically apply the secondary Multus configurations based on the specified VLAN parameters.
