...
export async function POST(request: Request) {
  try {
    ...
    const { action, jobId = 'default' } = body;

    const job = getJob(jobId);

    if (action === 'resume') {
      if (!job.params) {
        return NextResponse.json({ error: 'No existing deployment parameters found to resume.' }, { status: 400 });
      }
      ...
      const { projectId, clusterName, deployEdgeRouter, machineType, ipMode, billingAccountId, preDeployWorkloads, secondaryNetworks } = job.params;
      runDeploymentSequence(projectId, clusterName, deployEdgeRouter, machineType, ipMode, jobId, billingAccountId, preDeployWorkloads, true, secondaryNetworks).catch((err) => {
        console.error('Background deployment sequence error during resume:', err);
      });

      addAuditLog('Resume Deploy Cluster', clusterName, `Resumed automation deployment for ${clusterName} from ${job.currentStep}`, 'success');
      return NextResponse.json({ success: true, message: 'Deployment sequence resumed.', job: getJob(jobId) });
    }

    const { projectId, clusterName, deployEdgeRouter = false, machineType = 'n2-standard-32', ipMode = 'internal', billingAccountId = '', preDeployWorkloads = [], secondaryNetworks = [] } = body;
    ...
    runDeploymentSequence(projectId, clusterName, deployEdgeRouter, machineType, ipMode, jobId, billingAccountId, preDeployWorkloads, false, secondaryNetworks).catch((err) => {
