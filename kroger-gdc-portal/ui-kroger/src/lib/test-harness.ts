...
import { runDeploymentSequence, runDestroySequence, getJob, subscribeToJob, killJob } from './deployment-runner';
  ...
  try {
    ...
    if (fs.existsSync(pidFile)) {
      ...
      if (pidStr) {
        ...
        console.log(`Stopping detached background test harness daemon (Process Group: -${pid})...`);
        process.kill(-pid, 'SIGKILL'); // Kill the entire process group
      }
      fs.unlinkSync(pidFile);
    }
  } catch (e: any) {
    console.warn("Could not kill background daemon:", e.message);
  }
}
    ...
    try {
      ...
      if (!runProvisioning || isExistingCluster) {
        ...
        if (config.dryRun) {
          ...
          steps[0].logs.push(`[${new Date().toISOString()}] • Initializing Terraform backend configurations...`);
          await new Promise(r => setTimeout(r, 1500));
          steps[0].logs.push(`[${new Date().toISOString()}] • Applying Terraform bootstrap modules...`);
          await new Promise(r => setTimeout(r, 1500));
          steps[0].logs.push(`[${new Date().toISOString()}] • Preparing GKE Connect agent on gem-admin-ws...`);
          await new Promise(r => setTimeout(r, 1500));
          steps[0].logs.push(`[${new Date().toISOString()}] • Executing Ansible playbook create-cluster.yaml...`);
          await new Promise(r => setTimeout(r, 1500));
          steps[0].status = 'success';
          steps[0].durationMs = Date.now() - s1Start;
          steps[0].details = `Cluster ${clusterName} created and verified in ${Math.round(steps[0].durationMs / 1000)}s (MOCK).`;
          steps[0].logs.push('✅ GKE Connect Gateway credentials generated and K8s API reachable.');
        } else {
          const deploymentJobId = `deploy-harness-${Date.now()}`;
          activeJobId = deploymentJobId;

          // Subscribe to real-time logs from the real deployment runner
          const unsubscribe = subscribeToJob(deploymentJobId, (line) => {
            const levelEmoji = line.level === 'error' ? '❌' : line.level === 'warn' ? '⚠️' : line.level === 'success' ? '✅' : '•';
            steps[0].logs.push(`[${line.timestamp}] ${levelEmoji} ${line.message}`);
            saveTestHarnessReport(report);
          });
          ...
          try {
            await runDeploymentSequence(
              projectId,
              clusterName,
              false, // deployEdgeRouter
              'n2-standard-8', // machineType
              'internal', // ipMode
              deploymentJobId
            );
            ...
            steps[0].durationMs = Date.now() - s1Start;
            steps[0].details = `Failed during deployment: ${err.message}`;
            throw err;
          } finally {
            unsubscribe();
            activeJobId = null;
      ...
      if (!runTeardown || isExistingCluster) {
        ...
        if (config.dryRun) {
          await new Promise(r => setTimeout(r, 1500));
          steps[4].logs.push(`[${new Date().toISOString()}] • Initiating Terraform destroy sequence...`);
          await new Promise(r => setTimeout(r, 1500));
          steps[4].logs.push(`[${new Date().toISOString()}] • Purging dynamic billing logs & Cloud NAT routing rules...`);
          await new Promise(r => setTimeout(r, 1500));
          steps[4].status = 'success';
          steps[4].durationMs = Date.now() - s5Start;
          steps[4].details = 'All cloud resources cleanly decommissioned. Zero cost leakage (MOCK).';
          steps[4].logs.push('✅ Cluster workspace and GCP state bucket cleaned.');
        } else {
          const destroyJobId = `destroy-harness-${Date.now()}`;
          activeJobId = destroyJobId;

          // Subscribe to real-time logs from the real teardown runner
          const unsubscribe = subscribeToJob(destroyJobId, (line) => {
            const levelEmoji = line.level === 'error' ? '❌' : line.level === 'warn' ? '⚠️' : line.level === 'success' ? '✅' : '•';
            steps[4].logs.push(`[${line.timestamp}] ${levelEmoji} ${line.message}`);
            saveTestHarnessReport(report);
          });

          try {
            await runDestroySequence(
              projectId,
              clusterName,
              false, // deployEdgeRouter
              'n2-standard-8', // machineType
            ...
            steps[4].durationMs = Date.now() - s5Start;
            steps[4].details = `Failed during teardown: ${err.message}`;
            throw err;
          } finally {
            unsubscribe();
            activeJobId = null;
      ...
      if (emailAlerts && notifyOnError) {
        ...
        let failedStepLogsHtml = "";
        const failedStep = steps.find(s => s.status === 'failed');
        if (failedStep && failedStep.logs.length > 0) {
          const lastLogs = failedStep.logs.slice(-25).map(l => {
            return l.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
          }).join("\n");
          failedStepLogsHtml = `
            <div style="margin-top: 20px;">
              <h3 style="color: #f8fafc; font-size: 13px; margin-bottom: 8px; font-weight: bold;">📋 Last 25 Diagnostic Log Lines (${failedStep.name}):</h3>
              <pre style="background-color: #020617; border: 1px solid #334155; padding: 12px; border-radius: 8px; font-family: monospace; font-size: 11px; color: #e2e8f0; overflow-x: auto; max-height: 250px; white-space: pre-wrap; margin: 0; line-height: 1.4; text-align: left;">${lastLogs}</pre>
            </div>
          `;
        }
...
export function startTestHarnessDaemon(config: TestHarnessConfig): TestHarnessReport {
  const {
    projectId,
    clusterName = 'gdc-e2e-test-1',
    emailAlerts,
  ...
  // Create and save initial report showing 'running' state so UI immediately displays progress
  const initialSteps: TestHarnessStep[] = [
    { id: 'step-1', name: 'Phase 1: Infrastructure Provisioning (bmctl & Terraform)', status: 'pending', logs: [] },
    { id: 'step-2', name: 'Phase 2: Virtual Machine & Workload Ingestion (KubeVirt CRDs)', status: 'pending', logs: [] },
    { id: 'step-3', name: 'Phase 3: User-Selected Performance & Database Stress Benchmarks', status: 'pending', logs: [] },
    { id: 'step-4', name: 'Phase 4: AI Sentinel Watchdog Anomaly Audit', status: 'pending', logs: [] },
  ...
  const report: TestHarnessReport = {
    ...
    emailSentTo: emailAlerts || undefined,
    steps: initialSteps,
    summary: 'Spawning background E2E daemon process...'
  };
  saveTestHarnessReport(report);
  addAuditLog('E2E Test Harness', clusterName, `Spawning detached daemon for ${clusterName} in ${projectId}`, 'info');

  try {
    const rootDir = process.cwd();
    
    // 1. Compile ts files to /tmp/harness-build
    console.log("Compiling E2E daemon and dependencies...");
    execSync(`npx tsc src/lib/harness-daemon.ts --outDir /tmp/harness-build --module commonjs --target es2020 --moduleResolution node --skipLibCheck --esModuleInterop`, {
      cwd: rootDir
    });

    // 2. Open log streams for daemon stdout/stderr
    const out = fs.openSync('/tmp/gdc_harness_daemon.log', 'a');
    const err = fs.openSync('/tmp/gdc_harness_daemon_err.log', 'a');

    // 3. Spawn daemon process in its own process group (detached: true)
    const nodeModulesPath = path.join(rootDir, 'node_modules');
    const env = { ...process.env, NODE_PATH: nodeModulesPath };

    const child = spawn('node', [
      '/tmp/harness-build/harness-daemon.js',
      JSON.stringify({ ...config, jobId }) // override/pass down jobId
    ], {
      detached: true,
      cwd: rootDir,
      env,
    ...
    report.status = 'failed';
    report.summary = `Failed to start daemon: ${e.message}`;
    saveTestHarnessReport(report);
    addAuditLog('E2E Test Harness', clusterName, `FAILED to start daemon: ${e.message}`, 'error');
  }
