#!/bin/bash
# TITLE: enroll-keyfactor-csr-san.sh
# DESCRIPTION: Enrolls a Certificate Signing Request (CSR) using the KeyFactor system
#   referencing a newreq.csr. Also includes example to adding SAN DNS values if
#   required.
...
curl --location --insecure --request POST "$KEYFACTOR_API/Enrollment/CSR" \
...
--data "{
  ...
  `#SAN DNS Example. Configured for your architecture`
  \"SANs\": {
      \"DNS\": [
        \"appname.kroger.com\",
        \"hostname1.kroger.com\",
        \"hostname2.kroger.com\"
      ]
  },
\"Metadata\": {
      \"Group\": \"$GROUP_NAME\",
