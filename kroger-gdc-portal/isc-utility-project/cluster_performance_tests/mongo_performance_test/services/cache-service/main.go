...
func warmCache(pool *redisPool, keyspace, warmKeys, ttlSeconds int) {
	if keyspace <= 0 || warmKeys <= 0 {
		return
	}
	if warmKeys > keyspace {
		warmKeys = keyspace
	}
	...
	for i := 0; i < warmKeys; i++ {
		key := "perf:key:" + strconv.Itoa(i)
		// Store deterministic fake data to simulate a populated cache at startup.
		val := fmt.Sprintf("warm-value-%d", i)
		if _, err := pool.do("SET", key, val, "EX", strconv.Itoa(ttlSeconds)); err != nil {
			errs++
		}
	}
	log.Printf("cache warm-up complete: requested=%d keyspace=%d ttl=%ds errors=%d durationMs=%d", warmKeys, keyspace, ttlSeconds, errs, time.Since(start).Milliseconds())
}
...
func main() {
	...
	warmKeys := atoiDefault(env("CACHE_WARM_KEYS", strconv.Itoa(keyspace)), keyspace)
	ttlSeconds := atoiDefault(env("CACHE_WARM_TTL_SECONDS", "3600"), 3600)
	warmCache(pool, keyspace, warmKeys, ttlSeconds)
